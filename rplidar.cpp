...
/*
	...
	std::cout << "[FIFO] FAIL" << std::endl;
        if(-1 == mkfifo(FIFO_FROM_OPENCV, 0666) ){
	    perror("[ERROR] CANNOT MAKE FIFO");
	    exit(1);
	}
...
*/
	...
	//fd_from_opencv = open(FIFO_FROM_OPENCV, O_RDWR);
        if (-1 == read(fd_from_opencv, buff, BUFF_SIZE)){
	    perror("[ERROR] CANNOT READ");
	    exit(1);
	}
	//if (0 == read(fd_from_opencv, buff, BUFF_SIZE)){
...
while(true){
		...
		//std::cout<<"arduino : "<<buff_a<<std::endl;
	}
//close(handle);
}
//write(handle, buff_a, 1);
//std::cout<<"sending data : "<<buff_a<<std::endl;
    ...
    toptions.c_oflag = 0; //Enable implementation-defined output processing.
